...
class SessionHandler implements SessionHandlerInterface
{
    ...
    public function read($session_id)
    {
        try {
            ...
            if (!empty($sess_id)) {
                ...
                session_decode($sess_id['session_data']);
    ...
    public function write($session_id, $session_data = null)
    {
        try {
            ...
            if ($GLOBALS['request']->getMethod() == 'POST') {
                print_r(['session_data'=>$session_data]); die;
            }
            ...
            if (!empty($old_sess)) {
                ...
                if ($old_sess['session_data'] == 'empty') {
                    ...
                    if ($sess_data['session_data'] !== 'empty') {



                print_r(['the interior sess'=> $old_sess['session_data']]); die;
                        $result = $this->model->add($this->table, $sess_data);
                        print_r(['log sess'=> $sess_data['session_data']]); die;
            ...
            $err = 'Session Database Exception - ';
            print_r(['ecept raised' => $err . $e->getMessage()]); die;
